...
enum Rule {
    Char(Char),
    List(Vec<usize>),
    DoubleList(Vec<usize>, Vec<usize>),
}

fn part1(raw_input: &'static str) -> u64
{
    let mut parts = raw_input.trim().split("\n\n");
    let rules: HashMap<usize, Rule> = parts.next().unwrap().lines().map(|line| {
        let mut parts = line.trim().split(": ");
        let index: usize = parts.next().unwrap().parse().unwrap();
        let rule_as_string = parts.next().unwrap();

        let rule = if rule_as_string == "\"a\"" {
            Rule::Char(Char::A)
        } else if rule_as_string == "\"b\"" {
            Rule::Char(Char::B)
        } else if rule_as_string.contains("|") {
            let mut parts = rule_as_string.split(" | ");
            Rule::DoubleList(
                parts.next().unwrap().split(' ').map(|one_char| one_char.parse().unwrap()).collect(),
                parts.next().unwrap().split(' ').map(|one_char| one_char.parse().unwrap()).collect(),
            )
        } else {
            Rule::List(
                rule_as_string.split(' ').map(|one_char| one_char.parse().unwrap()).collect()
            )
        };
    ...
    let messages: Vec<Vec<Char>> = parts.next().unwrap().lines().map(|line| {
        line.trim().chars().map(|one_char| {
            match one_char {
                'a' => Char::A,
                'b' => Char::B,
                _ => panic!("Unknown char {}", one_char),
            }
        }).collect()
    }).collect();
    ...
    for message in messages {
        let (matched, last_index) = message_match(&rules, &message, 0, 0);
        if matched && last_index == message.len() {
            result += 1;
        }
    }
...
fn message_match(rules: &HashMap<usize, Rule>, message: &Vec<Char>, index: usize, rule_index: usize) -> (bool, usize)
{
    ...
    match rule {
        ...
        Rule::List(list) => {
            match_list(rules, message, index, &list)
        },
        Rule::DoubleList(list_a, list_b) => {
            let (result, new_index) = match_list(rules, message, index, &list_a);

            if result {
                (result, new_index)
            } else {
                match_list(rules, message, index, &list_b)
            }
        },
    }
}

fn match_list(rules: &HashMap<usize, Rule>, message: &Vec<Char>, index: usize, list: &Vec<usize>) -> (bool, usize)
{
    let mut new_index = index;
    for other_rule_index in list {
        let (result, new_new_index) = message_match(rules, message, new_index, *other_rule_index);
        if result {
            new_index = new_new_index;
        } else {
