...
fn part1(raw_input: &'static str) -> u64
{
    ...
    for line in raw_input.trim().lines() {
        if line.trim().starts_with("mask = ") {
           ...
           let (new_zeros, new_ones) = mask_from_string(mask_as_string);
           zeros = new_zeros;
           ones = new_ones;
        } else {
            ...
            let address: u64 = (&mem[4..mem.len() - 1]).parse().unwrap();
            
            values.insert(address, apply_mask(value, zeros, ones));
        }
    }
...
fn test_apply_mask()
{
    let (zeros, ones) = mask_from_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X");
    assert_eq!(73, apply_mask(11, zeros, ones));

    let (zeros, ones) = mask_from_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X");
    assert_eq!(101, apply_mask(101, zeros, ones));

    let (zeros, ones) = mask_from_string("XXXXXXXXXXXXXXXXXXXXXXXXXXXXX1XXXX0X");
    assert_eq!(64, apply_mask(0, zeros, ones));
}

#[test]
fn test_mask_from_string()
{
    let (zeros, ones) = mask_from_string("1001");
    assert_eq!(u64::MAX - 6, zeros);
    assert_eq!(9, ones);
