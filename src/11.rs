...
fn part1(raw_input: &'static str) -> usize
{
    ...
    for line in raw_input.trim().lines() {
        ...
        for position in line.trim().chars() {
            match position {
                'L' => {
                    seats.insert(Position { column: column_index, line: line_index });
                },
                '.' => {},
        ...
        for seat in &seats {
            ...
            for line_diff in &[-1, 0, 1] {
                for column_diff in &[-1, 0, 1] {
                    ...
                    if occupied_seats.contains(&Position { column: seat.column + column_diff, line: seat.line + line_diff }) {
