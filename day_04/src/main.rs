...
fn input(raw_input: &'static str) -> Vec<Passport> {
    raw_input
        ...
        .filter_map(|raw_passport| {
            ...
            for key_value_string in raw_passport.split_whitespace() {
                let mut key_value = key_value_string.split(':');
            ...
            match (byr, iyr, eyr, hgt, hcl, ecl, pid, cid) {
                (Some(byr), Some(iyr), Some(eyr), Some(hgt), Some(hcl), Some(ecl), Some(pid), cid) => Some(Passport {
